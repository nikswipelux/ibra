...
@can('cmc_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.cmcs.create') }}">
                ...
                <tbody>
                    @foreach($cmcs as $key => $cmc)
                        <tr data-entry-id="{{ $cmc->id }}">
                            ...
                            <td>
                                @can('cmc_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.cmcs.show', $cmc->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('cmc_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.cmcs.edit', $cmc->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('cmc_delete')
                                    <form action="{{ route('admin.cmcs.destroy', $cmc->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('cmc_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.cmcs.massDestroy') }}",
