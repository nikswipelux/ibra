...
@can('playtoearn_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.playtoearns.create') }}">
                ...
                <tbody>
                    @foreach($playtoearns as $key => $playtoearn)
                        <tr data-entry-id="{{ $playtoearn->id }}">
                            ...
                            <td>
                                @can('playtoearn_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.playtoearns.show', $playtoearn->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('playtoearn_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.playtoearns.edit', $playtoearn->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('playtoearn_delete')
                                    <form action="{{ route('admin.playtoearns.destroy', $playtoearn->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('playtoearn_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.playtoearns.massDestroy') }}",
